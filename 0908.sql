...
-- 고객테이블에서 고객이름이 김씨이고 고객아이디에 a문자가 포함되어 있고 등급에 i문자가 포함된 고객의 고객아이디, 고객이름, 등급, 직업을 검색하시오.
...
-- 고객테이블에서 나이가 아직 입력되지 않은 고객의 고객이름, 나이를 검색하시오.
...
-- 고객테이블에서 나이가 25세 이상인 고객의 고객이름, 나이를 검색하시오.
select 고객이름, 나이 from 고객 where 나이 >= 25;
...
-- 주문테이블에서 p03 또는 p06의 주문제품을 주문하고 배송지가 경기도인 주문을 검색하시오. 단 주문수량을 기준으로 내림차순 정렬하시오.
...
-- 소수점 세째자리에서 반올림해서 둘째자리까지 출력, round(avg(단가), 2)
select * from 제품;
